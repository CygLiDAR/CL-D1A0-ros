...
using namespace std;
namespace cld1a0_pcl_driver 
{
    cld1a0_pcl::cld1a0_pcl(const std::string& port, uint32_t baud_rate, boost::asio::io_service& io)
    ...
    uint8_t* cld1a0_pcl::poll(int version_num)
    {
    ...
    void cld1a0_pcl::packet(int version_num)
    {
        boost::asio::write(serial_, boost::asio::buffer(PACKET_INTEGRATION_OFF));
        ROS_INFO("PACKET SENT (INTEGRATION_OFF)");
